...
@section('header_buttons')
    <div class="row">
        <div class="col-sm-12 d-flex justify-content-end mb-3">
            <a href="{{ url('admin/sports') }}" class="btn btn-primary">
...
@section('content')
    ...
    <form method="POST" action="{{ route('backend.sports.store') }}">
        @csrf
        <input type="hidden" name="id" value="{{ isset($sport) ? $sport->id : '' }}">
        <input type="hidden" id="temp_id" name="temp_id" value="{{ $temp_id }}">
        <div class="row">
            <div class="col-sm-8">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Sport Details</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-12 mb-4">
                                <label>Slug</label>
                                <input class="form-control" type="text" id="slug" name="slug" placeholder="Enter here...." value="{{ isset($sport) ? $sport->slug : '' }}" readonly>
                                <label class="text-danger fw-bold mt-1 d-none" id="slug-warning">Slug already exists!</label>
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Title (ENGLISH) *</label>
                                <input class="form-control" type="text" id="main-title" name="en_title" placeholder="Enter here...." value="{{ isset($sport) ? $sport->en_title : '' }}">
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Content (ENGLISH)</label>
                                <textarea id="content-en" name="en_content">
                                    {{ isset($sport) ? $sport->en_content : '' }}
                                </textarea>
                            </div>
                            ...
                            <div class="col-sm-12 mb-4">
                                ...
                                <input class="form-control" type="text" id="title-si" name="si_title" placeholder="Enter here...." value="{{ isset($sport) ? $sport->si_title : '' }}">
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Content (SINHALA)</label>
                                <textarea id="content-si" name="si_content">
                                    {{ isset($sport) ? $sport->si_content : '' }}
                                </textarea>
                            </div>
                            ...
                            <div class="col-sm-12 mb-4">
                                ...
                                <input class="form-control" type="text" id="title-ta" name="ta_title" placeholder="Enter here...." value="{{ isset($sport) ? $sport->ta_title : '' }}">
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Content (TAMIL)</label>
                                <textarea id="content-ta" name="ta_content">
                                    {{ isset($sport) ? $sport->ta_content : '' }}
                                </textarea>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-5">
                            <div class="col-sm-12">
                                ...
                                <select class="form-control" name="sport_category_id">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ (!empty($sport) && $sport->sport_category_id == $category->id) ? 'selected' : '' }}>{{ $category->sport_category }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                                ...
                                                @foreach($images as $img )
                                                    <div class="col-md-6 mb-3" id="row-{{ $img->id }}">
                                                        <div class="d-flex align-items-center justify-content-center img-action img-border">
                                                            <img class="img-fluid img-bordered" src="{{ url('assets/common/images/uploads/sports/'.$img->image) }}">
                                                            @if( $img->is_primary == 1 )
    ...
    <input type="hidden" id="route_get_slug" value="{{ route('backend.sports.slugGenerator') }}">
...
@section('custom_scripts')
    <script>
        ...
        function appendImage($img){
            ...
            $image = "{{ url('assets/common/images/uploads/sports') }}/" + $img.filename;
        ...
        $('.thumb_crop').click(function(event){
            ...
            $image_thumb.croppie('result', {
                ...
                $.ajax({
                    url: "{{ route('backend.sports.imageUpload') }}",
                    type: "POST",
                    data: {
                        image:response,
                        id: $id,
            ...
            $('#uploaded_image').on('click', '.delete-image', function ($e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            $.ajax({
                                url: "{{ route('backend.sports.deleteImage') }}",
                                type: 'POST',
                                data: {
                                    id: $id,
                                    _token: csrf_token()
            ...
            $('#uploaded_image').on('click', '.primary-image', function ($e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            $.ajax({
                                url: "{{ route('backend.sports.setPrimaryImage') }}",
                                type: 'POST',
                                data: {
                                    id: $id,
                                    _token: csrf_token()
