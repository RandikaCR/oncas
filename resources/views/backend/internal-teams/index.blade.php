...
@php
    ...
    $singlePageTitle = 'Internal Team';
    $routePrefix = 'internalTeams';
    $pageUrl = 'internal-teams';
@endphp
                                    ...
                                    <table class="table table-hover table-striped align-middle table-nowrap mb-0">
                                        ...
                                        @foreach($records as $row)
                                            <tr id="row-{{ $row->id }}">
                                                ...
                                                <td>
                                                    <p class="mb-0">{{ $row->internal_team }}</p>
                                                </td>
        ...
        $(document).ready(function (){
            $('.save-this-form').on('click', function (){
                ...
                if($name != ''){
                    ...
                    $('#form-alert-area').html('');
                    $alert = alertDanger('Internal Team can not be empty!', 'Error');
                    $('#form-alert-area').html($alert);
                    $($this).prop('disabled', false);
                }
            ...
            $('.table').on('click', '.edit', function (){
                ...
                $.ajax({
                    ...
                    success: function ($res, $textStatus, $jqXHR) {
                        $('#edit-id').val($res.id);
                        $('#name-input').val($res.internal_team);
                        $('#form-alert-area').html('');
