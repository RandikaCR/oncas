                    ...
                    <form method="GET" action="">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3 d-flex align-items-end">
                                ...
                                <a href="{{ url('admin/events') }}" class="btn btn-outline-dark waves-effect waves-light ms-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="live-preview">
                                            ...
                                            <tr id="row-{{ $event->id }}">
                                                ...
                                                <td>
                                                    <div class="bg-light rounded p-1">
                                                        <img src="{{ asset('assets/common/images/uploads/events/' .$event->primary_image) }}" class="img-fluid d-block" alt="Img">
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="mb-1">{{ $event->en_title }}</p>
                                                    <p class="mb-1 text-muted">{{ $event->si_title }}</p>
                                                    <p class="mb-0">{{ $event->ta_title }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="mb-0">{{ $event->event_category->event_category }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="mb-0"><span class="badge {{ $event->status()->class }}">{{ $event->status()->text }}</span></p>
                                                </td>
                                                <td class="text-end">
                                                    <div class="d-flex justify-content-end align-items-center">
                                                        <div class="form-check form-switch form-switch-success form-switch-md">
                                                            <input class="form-check-input status" data-id="{{ $event->id }}" type="checkbox" role="switch"  {{ ($event->status == 1) ? 'checked': '' }} >
                                                        </div>
                                                        <div>
                                                            <a href="{{ url('events/' . $event->slug) }}" target="_blank" class="btn btn-primary btn-sm waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><span class="mdi mdi-magnify"></span></a>
                                                            <a href="{{ route('backend.events.edit', $event->uuid) }}" class="btn btn-primary btn-sm waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><span class="mdi mdi-pencil"></span></a>
                                                            <a href="javascript:void(0);" class="btn btn-danger btn-sm waves-effect waves-light delete" data-id="{{ $event->id }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"><span class="mdi mdi-delete"></span></a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                        ...
                        <div class="mt-5">
                            {{--Paginaiton--}}
                            {!! $events->links('vendor.pagination.backend') !!}
                        </div>
                    </div>
                </div><!-- end card-body -->
            </div><!-- end card -->
    ...
    <script>
        $(document).ready(function (){
            ...
            $('.table').on('change', '.status', function (){
                $id = $(this).data('id');
                $url = "{{ route('backend.events.status') }}";
                $rowId = '#row-' + $id;
                $.ajax({
                    url: $url,
                    dataType: 'json',
