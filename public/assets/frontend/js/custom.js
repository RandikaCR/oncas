...
window.addEventListener("load", function () {
  ...
  document.body.style.overflow = "visible"
    ...
    const featCols = document.querySelectorAll('.feat-col');
    const bgMain = document.querySelector('.feat-bg-main');

    // Function to update the view
    const updateFeature = (el) => {
        const newImg = el.getAttribute('data-bg');
        
        // 1. Update background image
        bgMain.style.backgroundImage = `url('${newImg}')`;
        
        // 2. Manage active class for text visibility
        featCols.forEach(col => col.classList.remove('active'));
        el.classList.add('active');
    };

    // Initialize with the first item
    const defaultActive = document.querySelector('.feat-col.active') || featCols[0];
    if (defaultActive) updateFeature(defaultActive);

    // Add hover listeners
    featCols.forEach(col => {
        col.addEventListener('mouseenter', () => updateFeature(col));
    });
/**
 * Scroll to Top
 */
